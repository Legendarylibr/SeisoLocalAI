...
from seiso.adaptive_quant.analysis_utils import ensure_directory, write_scatter_plot
from seiso.adaptive_quant.configuration.validation import validate_cli_path_argument
from seiso.adaptive_quant.logging_utils import read_json, write_json
from seiso.adaptive_quant.math_utils import mean
from seiso.analysis.log_records import (
...
def analyze_frontier(eval_path: str, output_dir: str) -> dict[str, object]:
    payload = read_json(eval_path, label="Frontier eval file")
    if not isinstance(payload, dict):
        raise TypeError("Frontier eval file must be a JSON object")
    eval_block = payload.get("eval")
    if not isinstance(eval_block, dict):
        raise ValueError("Frontier eval file missing top-level 'eval' object")
    overlap_by_domain = eval_block.get("overlap_by_domain")
    domain_chart: dict[str, float] = {}
    if isinstance(overlap_by_domain, dict):
        domain_chart = {
            str(key): float(value)
            for key, value in overlap_by_domain.items()
    ...
    if isinstance(rows, list):
        for row in rows:
            ...
            if overlap is None or length_ratio is None:
                continue
            scatter_points.append((float(length_ratio), float(overlap)))
    summary: dict[str, object] = {
        "eval_path": eval_path,
        "passed": eval_block.get("passed"),
        "prompts_evaluated": eval_block.get("prompts_evaluated"),
        "mean_reference_overlap": eval_block.get("mean_reference_overlap"),
        "median_reference_overlap": eval_block.get("median_reference_overlap"),
        "local_coverage_rate": eval_block.get("local_coverage_rate"),
        "failed_prompt_ids": eval_block.get("failed_prompt_ids"),
        "thresholds": eval_block.get("thresholds"),
    }
    output_root = ensure_directory(output_dir)
    bar_charts = (
        [
            (
                "frontier_overlap_by_domain.svg",
                "Frontier Reference Overlap by Domain",
                domain_chart,
                "Overlap",
            )
        ]
    ...
    scatter_charts = (
        [
            (
                "frontier_length_ratio_vs_overlap.svg",
                "Local/Frontier Length Ratio vs Overlap",
                scatter_points,
                "Length Ratio",
                "Overlap",
            )
    ...
    return _publish_analysis(
        output_root,
        summary,
        json_name="frontier_eval_summary.json",
        bar_charts=bar_charts,
        scatter_charts=scatter_charts,
    )
...
_CLI: dict[str, tuple[str, object, str, bool]] = {
    "hardware_generalization": (
        "python -m seiso.analysis hardware_generalization <log_path> <output_dir> [--phase eval|train|all]",
        analyze_hardware,
        "Wrote hardware analysis to",
        True,
    ),
    "input_adaptation": (
        "python -m seiso.analysis input_adaptation <log_path> <output_dir> [--phase eval|train|all]",
        analyze_inputs,
        "Wrote input adaptation analysis to",
        True,
    ),
    "moe_cache_behavior": (
        "python -m seiso.analysis moe_cache_behavior <log_path> <output_dir> [--phase eval|train|all]",
        analyze_moe_cache,
        "Wrote MoE cache analysis to",
        True,
    ),
    "moe_expert_behavior": (
        "python -m seiso.analysis moe_expert_behavior <log_path> <output_dir> [--phase eval|train|all]",
        analyze_moe_experts,
        "Wrote MoE expert analysis to",
        True,
    ),
    "quant_function_behavior": (
        "python -m seiso.analysis quant_function_behavior <log_path> <output_dir> [--phase eval|train|all]",
        analyze_quant,
        "Wrote quant function analysis to",
        True,
    ),
    "training_dynamics": (
        "python -m seiso.analysis training_dynamics <history_path> <output_dir>",
        analyze_training_dynamics,
        "Wrote training dynamics analysis to",
        False,
    ),
    "online_learning": (
        "python -m seiso.analysis online_learning <log_path> <output_dir>",
        analyze_online,
        "Wrote online analysis to",
        False,
    ),
    "frontier_eval": (
        "python -m seiso.analysis frontier_eval <eval_json_path> <output_dir>",
        analyze_frontier,
        "Wrote frontier eval analysis to",
        False,
    ),
}

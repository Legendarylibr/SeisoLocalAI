...
Also composes shared funding discovery (Ark + L402 + x402 + faucet). Live
Ark, Lightning L402, and x402 EVM are **not functional yet** — do not use
for real funds. ``SEISO_PAY_L402_SIM=1`` / ``SEISO_PAY_X402_SIM=1`` (or
faucet) enable simulated fund/exchange.
"""
...
SettleMode = Literal["faucet", "ark", "simulated", "l402", "x402"]
...
def funding_instructions(session_id: str, amount_sats: int) -> dict[str, Any]:
    """Payment instructions for buyers (Ark, L402, x402, and/or faucet)."""
    ...
    if x402_sim:
        extras.append(
            "Simulated x402 EVM fund/exchange is available (POST /pay/v1/sessions/fund/x402)."
        )
    out: dict[str, Any] = {
        ...
        "detail": (
            ...
            + (" ".join(extras) if extras else "Faucet/sim only for local smoke tests.")
        ),
    }
    if faucet_enabled():

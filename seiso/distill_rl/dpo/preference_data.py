...
from seiso.distill_rl.dpo.json_utils import (
...
def load_preference_dataset(path: str | Path) -> list[dict[str, str]]:
        ...
        with source.open(encoding="utf-8") as handle:
            for line_no, line in enumerate(handle, start=1):
                ...
                if len(line.encode("utf-8")) > MAX_JSONL_LINE_BYTES:
                    raise ValueError(
                        f"JSONL line exceeds byte limit ({MAX_JSONL_LINE_BYTES}): "
                        f"line {line_no} in {source}"
                    )
                row = safe_json_loads(line, label=f"{source}:{line_no}")
                enforce_safe_parsed_json(row, label=f"{source}:{line_no}")
    ...
    if isinstance(payload, dict) and isinstance(payload.get("examples"), list):
        return [
            _normalize_preference_row(row, label=source) for row in payload["examples"]
        ]
    raise ValueError(

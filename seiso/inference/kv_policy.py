...
from seiso.inference.torch_load_policy import (
    TorchLoadPolicy as TorchLoadPolicy,
)
from seiso.inference.torch_load_policy import (
    estimate_local_weight_mb as estimate_local_weight_mb,
)
from seiso.inference.torch_load_policy import (
    resolve_torch_load_policy as resolve_torch_load_policy,
)
...
def resolve_kv_cache_policy(
    ...
    *,
    ...
    chunk_override = env_str("SEISO_TORCH_PREFILL_CHUNK_SIZE", "").strip()
    if chunk_override:
        try:
            configured_chunk = max(128, int(chunk_override))
        except ValueError:
            configured_chunk = 1024
    elif usable >= 16 * 1024 and (estimated <= 0 or estimated < usable * 0.25):
        configured_chunk = 2048
    elif usable >= 8 * 1024:
        configured_chunk = 1024
    else:
        configured_chunk = 512

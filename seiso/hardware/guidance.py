...
def build_guidance(
    goal: str, *, backend: Backend, gpus: list[dict], ram_gb: float
) -> list[GuideStep]:
    ...
    if goal == "chat":
        if vram_total >= 12000 or backend == Backend.MLX:
            steps.append(
                GuideStep(
                    "Download a 7–14B model",
                    "Your hardware can run strong chat models locally.",
                    "/hub",
                    3,
                )
            )
            steps.append(
                GuideStep(
                    "Open Chat",
                    "Start a new conversation with encrypted session memory.",
                    "/chat",
                    2,
                )
            )
        ...
        if vram_total >= 24000:
            ...
            steps.append(
                GuideStep(
                    "Consider QLoRA or cloud export",
                    "Training needs more VRAM — try compress or export flows.",
                    "/compress",
                    2,
                )
            )
        if ram_gb < 16:
            steps.append(
                GuideStep(
                    "Close other apps",
                    f"System RAM is {ram_gb} GB — training benefits from 16 GB+.",
                    "/",
                    1,
                )
            )
        ...
        steps.append(
            GuideStep(
                ...
                "Prune, quantize, and export smaller models.",
                "/compress",
                3,
            )
        )
        if vram_total < 8000:
            steps.append(
                GuideStep(
                    "Start small",
                    "Use smoke presets on CPU if GPU memory is tight.",
                    "/compress",
                    2,
                )
            )
        ...
        steps.append(
            GuideStep(
                "Browse models",
                "Download a GGUF from the Hub — newest models listed first.",
                "/hub",
                3,
            )
        )
        ...
        if vram_total < 6000 and not gpus:
            steps.append(
                GuideStep(
                    "Download a Q4 GGUF",
                    "llama.cpp runs small quantized models efficiently on CPU.",
                    "/hub",
                    2,
                )
            )

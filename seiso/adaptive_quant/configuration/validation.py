...
_LLAMA_CPP_MODEL_PREFIXES_ENV = "ADAPTIVE_RL_LLAMA_CPP_MODEL_PREFIXES"
_BLOCKED_FRONTIER_HOSTS = frozenset(
    {
        "169.254.169.254",
...
def validate_backend(name: str) -> None:
    ...
    if key in _BACKENDS:
        return
    from seiso.adaptive_quant.backends.registry import _EXTRA_BUILDERS
...
def validate_quant_mode(name: str) -> None:
    from seiso.adaptive_quant.types import QuantMode
...
def validate_llama_cpp_model_allowlist(resolved_model: str) -> None:
    """When ``ADAPTIVE_RL_LLAMA_CPP_MODEL_PREFIXES`` is set, require the model under those roots."""
    raw = os.environ.get(_LLAMA_CPP_MODEL_PREFIXES_ENV, "").strip()
    if not raw:
        return
    prefixes: list[str] = []
    for entry in raw.split(os.pathsep):
        ...
        try:
            ...
            raise ValueError(
                f"{_LLAMA_CPP_MODEL_PREFIXES_ENV} contains invalid prefix {entry!r}: {exc}"
            ) from exc
    if not prefixes:
        return
    if not any(
        resolved_model == prefix or resolved_model.startswith(prefix + os.sep)
        for prefix in prefixes
    ):
        raise ValueError(
            f"llama_cpp_model resolves to {resolved_model!r}, which is outside "
            f"{_LLAMA_CPP_MODEL_PREFIXES_ENV}={raw!r}"
        )
...
def validate_frontier_api_base_url(api_base: str) -> None:
    """Reject credential-bearing or cloud-metadata frontier endpoints."""
    if not isinstance(api_base, str) or not api_base.strip():
        raise ValueError("frontier_api_base must be a non-empty URL")
    parsed = urlparse(api_base.strip())
    if parsed.username or parsed.password:
        raise ValueError("frontier_api_base must not include embedded credentials")
    host = (parsed.hostname or "").strip().lower()
    if not host:
        raise ValueError("frontier_api_base must include a hostname")
    if host in _BLOCKED_FRONTIER_HOSTS:
        raise ValueError(f"frontier_api_base host is not allowed: {host!r}")
    try:
...
def validate_frontier_settings(
    *,
    ...
    comparison_prompt_limit: int,
    comparison_metric: str,
    eval_min_overlap: float,
    eval_min_local_coverage: float,
) -> None:
    if not isinstance(model, str) or not model.strip():
        raise ValueError("frontier_model must be a non-empty string")
    if len(model.strip()) > 128:
        raise ValueError("frontier_model exceeds 128 characters")
    if not isinstance(api_base, str) or not api_base.strip():
        raise ValueError("frontier_api_base must be a non-empty URL")
    ...
    if not (
        ...
        raise ValueError("frontier_api_base must use https:// or localhost http://")
    validate_frontier_api_base_url(api_base)
    for name, value in (
        ("frontier_api_key_env", api_key_env),
        ("frontier_fallback_api_key_env", fallback_api_key_env),
        ("frontier_comparison_metric", comparison_metric),
    ):
        if not isinstance(value, str) or not value.strip():
            raise ValueError(f"{name} must be a non-empty string")
    metric_name = comparison_metric.strip()
    if metric_name != "reference_overlap":
        raise ValueError(
            "frontier_comparison_metric must be 'reference_overlap' "
            f"(only supported metric), got {metric_name!r}"
        )
    ...
    if not 0.0 <= overlap <= 1.0:
        raise ValueError("frontier_eval_min_overlap must be in [0, 1]")
    if not isinstance(eval_min_local_coverage, (int, float)) or isinstance(
        eval_min_local_coverage, bool
    ):
        raise TypeError("frontier_eval_min_local_coverage must be numeric")
    coverage = float(eval_min_local_coverage)
    if not 0.0 <= coverage <= 1.0:
        raise ValueError("frontier_eval_min_local_coverage must be in [0, 1]")

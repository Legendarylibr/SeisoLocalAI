"""Grouped configuration sections composed by :class:`~seiso.adaptive_quant.configuration.FrameworkConfig`."""
...
class FrontierSettings:
    """Hosted frontier-model reference for comparing local llama.cpp/simulator outputs."""

    enabled: bool = False
    model: str = "gpt-4o-mini"
    api_base: str = "https://api.openai.com/v1"
    api_key_env: str = "ADAPTIVE_RL_FRONTIER_API_KEY"
    fallback_api_key_env: str = "OPENAI_API_KEY"
    max_tokens: int = 512
    temperature: float = 0.2
    timeout_s: float = 120.0
    max_prompt_chars: int = 8192
    comparison_prompt_limit: int = 12
    auto_compare_in_pipeline: bool = False
    write_external_quality_sidecar: bool = True
    comparison_metric: str = "reference_overlap"
    local_reference_path: str | None = None
    eval_min_overlap: float = 0.25
    eval_min_local_coverage: float = 0.50
    eval_write_analysis: bool = True


@dataclass
class ContinuousLearningSettings:
    """Long-horizon RL over a streaming task sequence (continuous tasks over time)."""

    enabled: bool = False
    max_tasks: int = 50_000
    task_stream_mode: str = "library_cycle"
    task_jsonl_path: str | None = None
    update_every_n_tasks: int = 1
    eval_every_n_tasks: int = 500
    checkpoint_every_n_tasks: int = 2_000
    replay_capacity: int = 4_096
    batch_size: int = 64
    min_replay_before_update: int = 8
    drift_window: int = 64
    drift_reward_delta: float = 3.0
...
class TrainingSettings:
    ...
    replay_verify_after_run: bool = False
...
_CONTINUOUS_FLAT: dict[str, tuple[str, str]] = {
    "continuous_learning_enabled": ("continuous", "enabled"),
}
for _field in ContinuousLearningSettings.__dataclass_fields__:
    if _field == "enabled":
        continue
    FLAT_FIELD_MAP[f"continuous_{_field}"] = ("continuous", _field)
FLAT_FIELD_MAP.update(_CONTINUOUS_FLAT)

_FRONTIER_FLAT: dict[str, tuple[str, str]] = {
    "frontier_enabled": ("frontier", "enabled"),
    "frontier_auto_compare_in_pipeline": ("frontier", "auto_compare_in_pipeline"),
}
for _field in FrontierSettings.__dataclass_fields__:
    if _field in ("enabled", "auto_compare_in_pipeline"):
        continue
    FLAT_FIELD_MAP[f"frontier_{_field}"] = ("frontier", _field)
FLAT_FIELD_MAP.update(_FRONTIER_FLAT)

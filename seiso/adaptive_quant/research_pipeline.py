...
def run_research_analysis(config: FrameworkConfig, history_path: str | None) -> dict[str, object]:
    from seiso.adaptive_quant.pipeline.output_summary import resolve_analysis_log_path
    from seiso.analysis.analyzers import (
        analyze_hardware,
        analyze_inputs,
        analyze_moe_cache,
        analyze_moe_experts,
...
class ResearchPipeline:
    ...
    def run(self) -> dict[str, object]:
        ...
        if frontier_summary is not None and not (
            isinstance(frontier_summary, dict) and frontier_summary.get("skipped")
        ):
            artifact_payload["frontier_comparison"] = config.frontier_comparison_path()
            artifact_payload["frontier_eval"] = config.frontier_eval_path()
            if isinstance(frontier_summary, dict) and frontier_summary.get(
                "external_quality_sidecar"
            ):
                artifact_payload["frontier_external_quality"] = frontier_summary[
                    "external_quality_sidecar"
                ]
        ...
        summary = {
            ...
            "frontier_comparison": frontier_summary,
            "analysis": analysis,
    ...
    def _resolve_config(self) -> tuple[FrameworkConfig, dict[str, object] | None]:
        ...
        from seiso.adaptive_quant.gpu_profiles import apply_gpu_profile
        from seiso.adaptive_quant.hardware import detect_cuda_device
        from seiso.adaptive_quant.torch_policy import (
    ...
    def _recommend_quantization(self, config: FrameworkConfig, trainer) -> dict[str, object]:
        from seiso.adaptive_quant.recommendation import recommend_quantization
...
def run_pipeline_entrypoint(
    ...
    *,
    ...
    from seiso.adaptive_quant.run_footer import print_launcher_dashboard_hint, print_pipeline_footer

...
def infer_simulator_engine(config: FrameworkConfig) -> str | None:
    if config.backend != "simulator":
        return None
    from seiso.adaptive_quant.rust_cli import rust_simulator_available
...
def _learning_components(config: FrameworkConfig) -> list[str]:
    if config.training_backend == "pytorch":
        base = ["TorchTrainer", "torch_policy", "base_trainer"]
    else:
        base = ["Trainer", "UniversalQuantizationPolicy", "base_trainer"]
    if config.router_enabled:

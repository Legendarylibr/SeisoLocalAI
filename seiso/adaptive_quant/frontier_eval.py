...
from seiso.adaptive_quant.frontier_comparison import compare_frontier_to_local
from seiso.adaptive_quant.logging_utils import write_json
from seiso.adaptive_quant.math_utils import mean
from seiso.adaptive_quant.prompts import PromptSample
...
def evaluate_frontier_vs_local(
    ...
    *,
    ...
    if config.frontier_eval_write_analysis:
        from seiso.analysis.analyzers import analyze_frontier

        analysis_root = f"{config.analysis_dir}/{config.run_name}/frontier"
        payload["seiso.analysis"] = analyze_frontier(eval_path, analysis_root)
        write_json(eval_path, payload)
    return {
        **comparison,
        "eval": eval_summary,

...
from seiso.adaptive_quant.kernel_rl import kernel_profile_count
from seiso.adaptive_quant.math_utils import discrete_precision_level
from seiso.adaptive_quant.types import QuantizationDecision, QuantMode
...
def torch_cuda_diagnostics(requested_device: str = "cuda") -> dict[str, Any]:
    ...
    if arch_list and not report["cuda_arch_supported"]:
        from seiso.adaptive_quant.torch_install import INSTALL_CUDA_TORCH_SCRIPT
...
def resolve_training_device(
    ...
    if device.type == "cuda":
        if not torch.cuda.is_available():
            ...
            if require_cuda:
                from seiso.adaptive_quant.torch_install import cuda_torch_install_instructions

...
def named_preset(name: str) -> FrameworkConfig:
    ...
    if key in ("continuous", "continuous_learning"):
        from seiso.adaptive_quant.presets.continuous import CONFIG_CONTINUOUS

        return CONFIG_CONTINUOUS.clone()
    ...
    raise ValueError(
        ...
        f"Use: default, reproducible, pytorch, post_train, continuous, minimal (or pass no preset)."
    )
...
def load_config(path: str | Path, *, strict: bool = True) -> FrameworkConfig:
    ...
    (``default``, ``reproducible``, ``pytorch``, ``post_train``, ``continuous``, ``minimal``); remaining keys override it.

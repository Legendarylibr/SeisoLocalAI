...
from seiso.adaptive_quant.gpu_profiles import SIMULATOR_PROFILE_TUNING, infer_gpu_profile
from seiso.adaptive_quant.math_utils import clamp
from seiso.adaptive_quant.types import HardwareProfile, HardwareType

_NVIDIA_SMI_CANDIDATE_PATHS: tuple[str, ...] = (
    "/usr/bin/nvidia-smi",
...
def resolve_nvidia_smi_executable() -> str | None:
    """Return an executable ``nvidia-smi`` path, probing PATH and common driver locations."""
    for env_name in ("ADAPTIVE_RL_NVIDIA_SMI_PATH", "NVIDIA_SMI_PATH"):
    ...
    for raw in _NVIDIA_SMI_CANDIDATE_PATHS:
        candidate = Path(raw)
        if candidate.is_file() and os.access(candidate, os.X_OK):
            return str(candidate.resolve())
    return None

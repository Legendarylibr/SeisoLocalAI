...
from seiso.adaptive_quant.ui.browser import open_dashboard_url
from seiso.adaptive_quant.ui.security import validate_bind_host
from seiso.adaptive_quant.ui.server import serve_launcher
...
def ensure_launcher_running(
    *,
    ...
    if launcher_autostart_suppressed():
        return None
    from seiso.adaptive_quant.ui.browser import wait_for_launcher_ready
    ...
    spawn_detached_process(
        [
            resolved_python,
            "-m",
            "seiso.adaptive_quant.ui.launcher",
            "--detach",
            "--no-browser",
            "--port",
            str(resolved_port),
...
def main(argv: list[str] | None = None) -> int:
    ...
    warnings.warn(
        "seiso.adaptive_quant.ui is a legacy standalone dashboard. "
        "Prefer Seiso Forge (seiso forge) and the RL Quant page for integrated workflows.",
        DeprecationWarning,
        stacklevel=2,
    )
    parser = argparse.ArgumentParser(
        description="Adaptive RL Quantization launcher dashboard (local web UI, legacy)."
    )

...
from seiso.adaptive_quant.backends.quality import ExternalQualityScores, apply_external_quality
...
from seiso.adaptive_quant.configuration.validation import (
    ...
    validate_llama_cpp_model_allowlist,
...
def extract_llama_cpp_generated_text(raw_output: str, *, prompt_text: str = "") -> str:
    ...
    for line in raw_output.splitlines():
        lower = line.lower()
        if _TIMING_RE.search(lower):
            continue
        if "tok/s" in lower or "ms per token" in lower:
            continue
        if lower.startswith("llama") and "print_timings" in lower:
            continue
        kept_lines.append(line.rstrip())
    text = "\n".join(kept_lines).strip()
    prompt = sanitize_user_text(prompt_text or "").strip()
    if prompt and text.startswith(prompt):
        text = text[len(prompt) :].lstrip()
    return text.strip()


def run_llama_cpp_completion(
    ...
    *,
    ...
    """Run llama.cpp and return parsed perf metrics plus generated text."""
    ...
    generated = extract_llama_cpp_generated_text(raw_output, prompt_text=prompt_text)
...
def require_llama_cpp_paths(
    ...
    *,
    ...
    validate_llama_cpp_model_allowlist(model)

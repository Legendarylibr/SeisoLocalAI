...
def is_linux_nvidia_host() -> bool:
    ...
    if platform.system().lower() != "linux":
        return False
    from seiso.adaptive_quant.hardware import nvidia_smi_visible
...
def recommended_gpu_install_ack_env() -> str:
    """Return the env var name users should set for CUDA install on this host."""
    if detect_wsl2():
        return _ACK_WSL_ENV
    return _ACK_HOST_VENV_ENV


def apply_gpu_install_ack_from_cli() -> str:
    """Set the Tier-3/4 ack env var for the current process (CUDA install CLI opt-in)."""
    env_name = recommended_gpu_install_ack_env()
...
def _boundary_failure_message(*, context: str) -> str:
    return (
        ...
        "  python3 scripts/install_cuda_torch.py --accept-gpu-install  # same as export, one-shot\n\n"
...
__all__ = [
    "apply_gpu_install_ack_from_cli",

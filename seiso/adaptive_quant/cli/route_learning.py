...
Subcommands manage a JSON :class:`seiso.adaptive_quant.model_routes.RouteCatalog`, fetch entries
through ``huggingface-cli`` / ``hf``, and train / query a :class:`seiso.adaptive_quant.route_policy.RouteBandit`
...
from seiso.adaptive_quant.huggingface_cli import (
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    add_config_file_argument(
        eval_prompts_parser, help_suffix="Otherwise uses baseline preset defaults."
    )
...
def _cmd_download(catalog_path: Path, args: argparse.Namespace) -> None:
    ...
    if cli is None:
        ...
        from seiso.adaptive_quant.huggingface_cli import HuggingFaceCli

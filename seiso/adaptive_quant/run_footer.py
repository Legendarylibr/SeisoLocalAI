...
from seiso.adaptive_quant.pipeline.research_contract import infer_evidence_level


def print_launcher_dashboard_hint(
    *,
    port: int | None = None,
    when: str = "complete",
) -> None:
    """Print the local launcher URL. Autostart is optional and respects CI/suppress env vars."""
    from seiso.adaptive_quant.ui.browser import format_terminal_hyperlink
    from seiso.adaptive_quant.ui.launcher import (
        ensure_launcher_running,
        launcher_autostart_suppressed,
        launcher_dashboard_url,
        resolve_launcher_port,
    )

    resolved_port = resolve_launcher_port(port)
    url = launcher_dashboard_url(port=resolved_port)
    link = format_terminal_hyperlink(url)
    if when == "startup":
        headline = "Launcher dashboard — open while this run continues:"
    else:
        headline = "Launcher dashboard — click to open:"
    print()
    print(headline)
    print(f"  {url}")
    if link != url:
        print(f"  {link}")
    print("  Start/reopen: make ui  (or: adaptive-rl-quant-ui)")
    sys.stdout.flush()
    if when == "startup" and not launcher_autostart_suppressed():
        try:
            ensure_launcher_running(port=resolved_port)
        except Exception as exc:
            print(
                f"Warning: could not auto-start launcher ({exc}). Open {url} or run: make ui",
                file=sys.stderr,
            )
            sys.stderr.flush()
...
def print_continuous_footer(
    ...
    *,
    ...
    continuous_map = summary.get("continuous")
    evaluation_map = summary.get("evaluation")
    artifacts = summary.get("artifacts")
    continuous = continuous_map if isinstance(continuous_map, Mapping) else {}
    evaluation = evaluation_map if isinstance(evaluation_map, Mapping) else {}
    artifact_map = artifacts if isinstance(artifacts, Mapping) else {}

    rows: list[tuple[str, str]] = [
        ("run_name", config.run_name),
        ("training_backend", str(config.training_backend)),
        ("task_stream_mode", str(config.continuous_task_stream_mode)),
        ("tasks", str(continuous.get("tasks", ""))),
        (
            "continuous_mean_reward",
            format_display(continuous.get("mean_reward", 0.0), style="footer"),
        ),
        ("continuous_total_updates", str(continuous.get("total_updates", 0))),
        (
            "eval_mean_reward",
            format_display(evaluation.get("mean_reward", 0.0), style="footer"),
        ),
        ("summary_json", summary_path),
        ("continuous_detail_json", str(artifact_map.get("continuous_detail", ""))),
        ("telemetry_jsonl", str(artifact_map.get("continuous_telemetry", ""))),
    ]
    history_path = artifact_map.get("training_history")
    if history_path:
        rows.append(("training_history", str(history_path)))

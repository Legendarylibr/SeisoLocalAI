...
def test_provider_url_blocks_cgnat_shared_address_space():
    ...
    with pytest.raises(SecurityError):
        validate_provider_base_url("https://100.64.0.1:443/v1", provider_type="remote_chat")
...
def test_local_chat_rejects_public_https():
    ...
    with pytest.raises(SecurityError, match="loopback"):
        validate_provider_base_url("https://example.com/v1", provider_type="local_chat")
    with pytest.raises(SecurityError, match="loopback"):
...
def test_resolve_pinned_endpoint_pins_remote_host(monkeypatch):
    ...
    endpoint = resolve_pinned_endpoint("https://example.com/v1", provider_type="remote_chat")
...
def test_resolve_pinned_endpoint_pins_loopback_dns_names(monkeypatch):
    ...
    endpoint = resolve_pinned_endpoint("http://local-vllm.test:8000", provider_type="local_chat")
...
def test_pin_request_to_ip_preserves_host_and_sni_without_getaddrinfo_patch():
    ...
    pinned = pin_request_to_ip(request, host="example.com", pinned_ip="93.184.216.34")
